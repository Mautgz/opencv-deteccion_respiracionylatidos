...
cv2.namedWindow("buscando")
cap = cv2.VideoCapture(0, cv2.CAP_DSHOW)

# cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)
# cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)
# en caso de necesitar la configuracion de la resolucion en 720p para optimizar los recursos
...
ok, frame=cap.read()
if not ok:
    print('Fallo la lectura de la cámara')
    exit()
...
fig=plt.figure()
plt.grid(True) 
plt.ion()  #modo interactivo inicializado no es necesario plt.show
plt.xlabel('Time'), 
plt.ylabel('Amplitude') 

plt.title('Detección de la respiración')
...
while(flag):
    ...
    green = getColorAverage(roi, 1)  ## calculamos la media del coolor verde en el ROI
    if(idf>50):
        gsums.append(green)
    ...
    # salir cuando el programa finalice
    if cv2.getWindowProperty('buscando', cv2.WND_PROP_AUTOSIZE) < 1:
        cap.release()
        break
...
cv2.waitKey(1)
print("La ejecución del programa finalizó")